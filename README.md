...
- `shared/market.ts`는 모든 계층이 공유하는 내부 `Quote`, `Candle`, `InstrumentMeta`, `AnalysisContext` 타입을 정의한다.
- `technicalindicators`와 서버 유틸로 EMA20/50/200, RSI14, MACD(12,26,9), Bollinger Bands(20,2), ATR14, ADX/DMI, VWAP, Anchored VWAP, RVOL, RS Line을 계산한다.
- 세션은 거래소 시간대와 거래 캘린더 기준으로 계산하며, US equities 기본값은 regular session only다.
- `Opening Range`는 서버 1분봉 기준으로 계산하고, intraday는 `RVOL-TOD`, daily는 classic RVOL을 사용한다.
...
- 거래량, RSI, MACD, ATR, ADX/DMI, RVOL, RS Line 보조 패널
- EMA20/50/200, Bollinger Bands, VWAP, Anchored VWAP, PDH/PDL, Opening Range 오버레이
- `intraday / swing` 모드 전환
- `regular / extended` 세션 전환
- benchmark 선택(`SPY/QQQ`)
- sticky anchor 기반 Anchored VWAP
- 지표 on/off 토글
...
- 기간, 인터벌, 모드, 세션, benchmark, anchor 변경 시 `/api/candles`가 다시 호출된다.
- `INVALID_SYM` 같은 잘못된 심볼에서 에러 UI가 표시된다.
- early close day 세션 테스트가 있다.
